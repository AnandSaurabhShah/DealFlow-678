...
const sections = {
  products: {
    ...
    initial: { name: '', category: '', price: '', unit: 'unit', tax: '0', description: '' },
    fields: [
...
export default function AdminConfigPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNotice(`${section.label.slice(0, -1)} created.`)
    },
  })
  ...
  return <div className="grid items-start gap-5 xl:grid-cols-[220px_minmax(0,1fr)]">
    ...
    <div className="space-y-5">
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-5"><h2 className="font-display text-base font-bold">Add {section.label.toLowerCase().slice(0, -1)}</h2><p className="mt-1 text-[10px] text-slate-400">Simple MVP 1 configuration.</p></div><form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">{section.fields.map(([name, label, type = 'text']) => <label key={name} className="text-[10px] font-semibold">{label}<input required={name !== 'description' && name !== 'location'} name={name} type={type} min={type === 'number' ? '0' : undefined} step={type === 'number' ? '0.01' : undefined} value={form[name]} onChange={event => setForm(current => ({ ...current, [name]: event.target.value }))} className="mt-1.5 block w-full rounded-lg border border-slate-200 p-2.5 outline-none focus:border-violet-500" /></label>)}<div className="sm:col-span-2">{notice && <p role="status" className="mb-3 rounded-lg bg-emerald-50 p-3 text-xs text-emerald-700">{notice}</p>}{createMutation.isError && <p role="alert" className="mb-3 rounded-lg bg-red-50 p-3 text-xs text-red-700">{getApiError(createMutation.error, `Unable to create ${section.label.toLowerCase()}`)}</p>}<button disabled={createMutation.isPending} className="rounded-lg bg-violet-600 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-50">{createMutation.isPending ? 'Creating…' : `Create ${section.label.toLowerCase().slice(0, -1)}`}</button></div></form></section>
