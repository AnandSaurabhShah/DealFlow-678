...
export default function QuotationsPage() {
  ...
  const canEdit = quote => user.role === 'REP' && quote.status === 'DRAFT'
  ...
  return <section className="min-h-[500px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <div className="flex items-center justify-between gap-3 overflow-x-auto p-4">
      <div className="flex min-w-max gap-1">{filters.map(item => <button key={item} onClick={() => setFilter(item)} className={`rounded-lg px-3 py-2 text-[11px] ${filter === item ? 'bg-violet-100 font-semibold text-violet-700' : 'text-slate-500 hover:bg-slate-50'}`}>{item === 'ALL' ? 'All' : item.toLowerCase()}<span className="ml-1.5 rounded-full bg-white px-1.5 py-0.5 text-[9px]">{item === 'ALL' ? data.length : data.filter(quote => quote.status === item).length}</span></button>)}</div>
    ...
    {quotations.isSuccess && <QuoteTable quotes={visible} onSelect={user.role === 'REP' ? quote => canEdit(quote) && navigate(`/quotations/${quote.id}`) : undefined} />}
  </section>
}
