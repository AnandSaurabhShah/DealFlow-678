...
function BuilderForm({ initialQuotation, products }) {
  ...
  return <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex flex-col justify-between gap-3 border-t border-slate-100 p-5 sm:flex-row sm:items-center"><h3 className="text-xs font-semibold">Line items <span className="rounded-full bg-violet-50 px-1.5 py-0.5 text-[9px] text-violet-700">{lines.length}</span></h3><div className="flex gap-2"><select aria-label="Product" disabled={!products.length || !isEditable} className="max-w-[240px] rounded-lg border border-slate-200 bg-white px-2 text-[10px] disabled:bg-slate-50" value={selectedProductId} onChange={event => setSelectedProductId(event.target.value)}>{products.map(product => <option value={product.id} key={product.id}>{product.name} · {product.billingType === 'RECURRING' ? 'Monthly' : 'One-time'}</option>)}</select><button type="button" onClick={addProduct} disabled={!selectedProductId || !isEditable} className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-2 text-[10px] font-semibold disabled:opacity-50"><Icon name="plus" />Add</button></div></div>
