...
export default function QuotationsPage({ fulfillmentOnly = false, billingOnly = false }) {
  ...
  const workflowData = workflowOnly ? data.filter(quote => ['APPROVED', 'FULFILLED'].includes(quote.status)) : data
  ...
  return <section className="min-h-[500px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="flex items-center justify-between gap-3 overflow-x-auto p-4">
      <div className="flex min-w-max gap-1">{availableFilters.map(item => <button key={item} onClick={() => setFilter(item)} className={`rounded-lg px-3 py-2 text-[11px] ${filter === item ? 'bg-violet-100 font-semibold text-violet-700' : 'text-slate-500 hover:bg-slate-50'}`}>{item === 'ALL' && workflowOnly ? billingOnly ? 'All billing' : 'All fulfillment' : filterLabels[item]}<span className="ml-1.5 rounded-full bg-white px-1.5 py-0.5 text-[9px]">{item === 'ALL' ? workflowData.length : workflowData.filter(quote => quote.status === item).length}</span></button>)}</div>
