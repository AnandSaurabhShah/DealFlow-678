...
export default function InternalNegotiationPage() {
  ...
  return <div className="space-y-5">
    ...
    {!isSent ? <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-[9px] font-semibold tracking-widest text-slate-400">OPEN CUSTOMER PORTAL ACCESS</p><h3 className="mt-1 font-display text-lg font-bold">Send to customer</h3><p className="mt-2 max-w-2xl text-xs leading-relaxed text-slate-500">The customer will receive an email containing a direct link to this quotation. If no customer is linked yet, enter the email address used for their portal account.</p><div className="mt-5 flex max-w-xl flex-col gap-2 sm:flex-row"><input type="email" value={customerEmail} onChange={event => setCustomerEmail(event.target.value)} disabled={Boolean(quote.customerId)} placeholder={quote.customerId ? 'Linked customer account will be emailed' : 'Customer portal email'} className="min-w-0 flex-1 rounded-lg border border-slate-200 p-3 text-xs outline-none focus:border-violet-500 disabled:bg-slate-50" /><button onClick={doSend} disabled={send.isPending || (!quote.customerId && !customerEmail.trim())} className="rounded-lg bg-violet-600 px-5 py-3 text-xs font-semibold text-white hover:bg-violet-700 disabled:opacity-50">{send.isPending ? 'Sending…' : 'Send and email customer'}</button></div></section> : <>
