...
export default function InternalNegotiationPage() {
  ...
  const doSend = () => {
    setNotice('')
    send.mutate({ id: quote.id, customerId: customerId.trim() || quote.customerId }, { onSuccess: () => setNotice('Quotation sent. The customer can now review and negotiate it in the portal.') })
  }
  return <div className="space-y-5">
    ...
    {!isSent ? <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-[9px] font-semibold tracking-widest text-slate-400">OPEN CUSTOMER PORTAL ACCESS</p><h3 className="mt-1 font-display text-lg font-bold">Send to customer</h3><p className="mt-2 max-w-2xl text-xs leading-relaxed text-slate-500">This approved quotation has not been shared. Use its linked customer, or supply a customer ID if none is linked.</p><div className="mt-5 flex max-w-xl flex-col gap-2 sm:flex-row"><input value={customerId} onChange={event => setCustomerId(event.target.value)} placeholder={quote.customerId ? `Linked customer: ${quote.customerId}` : 'Customer ID (required)'} className="min-w-0 flex-1 rounded-lg border border-slate-200 p-3 text-xs outline-none focus:border-violet-500" /><button onClick={doSend} disabled={send.isPending || (!quote.customerId && !customerId.trim())} className="rounded-lg bg-violet-600 px-5 py-3 text-xs font-semibold text-white hover:bg-violet-700 disabled:opacity-50">{send.isPending ? 'Sending…' : 'Send to customer'}</button></div></section> : <>
