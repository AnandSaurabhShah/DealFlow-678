    ...
    <table className="w-full min-w-[640px] border-collapse text-left text-[11px]">
      ...
      <tbody>{rows.map((row, index) => {
        ...
        return <tr key={`${row.productId}-${row.warehouseId || 'unassigned'}-${index}`} className="border-b border-slate-100 last:border-0">
          <td className="px-4 py-3"><strong>{product?.name || 'Unknown product'}</strong><p className="mt-0.5 text-[9px] text-slate-400">{product?.category ? configEnumLabel(product.category) : row.productId}</p></td>
